...
def find_project_root(start: Path) -> Path:
    for candidate in [start.resolve(), *start.resolve().parents]:
        if (candidate / "pyproject.toml").exists() and (candidate / "src" / "covid_rars").exists():
            return candidate
    raise FileNotFoundError(
        "Could not find project root. Run this from the Covid-RARS repository root "
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fast local preflight for the COVID-RARS notebooks.")
