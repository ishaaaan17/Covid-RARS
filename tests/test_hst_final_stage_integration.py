...
def _pipeline(tmp_path: Path) -> SimpleNamespace:
    ...
    workspace = tmp_path / "covid_rars"
    workspace.mkdir()
    (workspace / "pyproject.toml").write_text(
        "[project]\nname = \"covid-rars\"\n", encoding="ascii"
    )
    (workspace / "src" / "covid_rars").mkdir(parents=True)
...
def _fusion_predictions(source_family: str) -> pd.DataFrame:
    import covid_rars.hst_fusion as fusion
...
def test_temporal_publication_table_applies_complete_case_uniform_fusion() -> None:
    import covid_rars.hst_stages as stages
    from covid_rars.hst_publication import authenticate_table, bind_analysis_plan
...
def test_gradcam_group_rows_use_all_correct_rows_and_cluster_by_participant() -> None:
    import numpy as np
    import covid_rars.hst_stages as stages
    from covid_rars.hst_gradcam import build_participant_gradcam_summary
...
def test_fusion_registry_binding_is_derived_from_actual_stage_receipts(
    ...
    assert verified["registry_authority"] == "covid_rars.HSTPipeline.receipt_chain"
