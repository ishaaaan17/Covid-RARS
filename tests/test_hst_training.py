...
REQUIRES_TORCH = pytest.mark.skipif(
    ...
    reason="Torch is optional and required only for checkpoint execution tests",
)
...
def test_training_config_rejects_wrong_effective_batch_and_unfrozen_full_mode() -> None:
    from covid_rars.hst_training import HSTTrainingConfig
...
def test_checkpoint_selection_contract_matches_primary_auroc_endpoint() -> None:
    from covid_rars.hst_training import HSTTrainingConfig, validation_epoch_score
...
def test_prediction_context_rejects_placeholder_or_missing_provenance() -> None:
    from covid_rars.hst_training import validate_prediction_context
...
def test_training_fingerprint_binds_manifest_cache_and_scientific_config() -> None:
    from covid_rars.hst_training import training_contract_fingerprint
...
def test_resume_checkpoint_contract_mismatch_fails_closed() -> None:
    from covid_rars.hst_training import validate_resume_checkpoint_contract
...
def test_fold_result_has_frozen_test_predictions_field() -> None:
    from covid_rars.hst_training import HSTFoldResult
...
def _registry_test_case(
    ...
    from covid_rars.hst_training import _collate, _model_architecture_sha256
...
def _run_registry_evaluation(
    ...
    *,
    ...
    from covid_rars.hst_training import _evaluate_split_once
...
def test_manifest_cache_contract_is_exact_and_fail_closed(tmp_path: Path) -> None:
    from covid_rars.hst_training import (
...
def test_manifest_cache_contract_validates_frozen_overlapping_cache_payload(
    ...
    from covid_rars.hst_training import validate_manifest_cache_contract
...
def test_tensor_sha_is_checked_before_loading(tmp_path: Path) -> None:
    from covid_rars.hst_training import load_verified_cached_image
...
def test_partial_or_incomplete_training_cannot_open_held_out_splits() -> None:
    from covid_rars.hst_training import validate_evaluation_request
...
def test_artifact_hashes_are_recomputed_from_files(tmp_path: Path) -> None:
    from covid_rars.hst_training import verify_training_artifact_hashes
...
def test_checkpoint_sidecar_is_required_before_pickle_load(tmp_path: Path) -> None:
    torch = pytest.importorskip("torch")
    from covid_rars.hst_training import _atomic_torch_save, _load_verified_checkpoint
...
def test_verified_checkpoint_loader_never_executes_forged_pickle_payload(
    ...
    torch = pytest.importorskip("torch")
    from covid_rars.hst_training import _load_verified_checkpoint_pair
    ...
    sidecar.write_text(
        json.dumps(
            {
                "schema_version": 2,
                "writer": "covid_rars.hst_training._atomic_torch_save",
...
def test_predict_split_refuses_mislabeled_loader_rows() -> None:
    ...
    from covid_rars.hst_training import _collate, predict_hst_split
...
def test_recorded_augmentation_seed_is_the_only_augmentation_driver() -> None:
    from covid_rars.hst_training import _augment_image_with_exact_seed
...
def test_executable_fingerprint_is_stable_when_repo_is_relocated(tmp_path: Path) -> None:
    from covid_rars.hst_training import _executable_files_sha256
...
def test_interrupted_held_out_evaluation_resumes_exact_anchored_source(
    ...
    import covid_rars.hst_training as training
...
def test_dataloaders_expose_external_split_and_do_not_persist_train_workers(
    ...
    from covid_rars.hst_training import make_hst_dataloaders
...
def test_training_loop_uses_ordered_draw_identity() -> None:
    from covid_rars.hst_training import train_hst_fold
...
def test_epoch_training_loss_is_weighted_by_sample_count() -> None:
    from covid_rars.hst_training import _sample_weighted_epoch_loss
...
def test_signal_handler_only_requests_optimizer_boundary_stop() -> None:
    ...
    from covid_rars.hst_training import _optimizer_boundary_signal_guard
...
def test_confirmatory_stage_adapter_passes_exact_evaluation_registry_keyword(
    ...
    from covid_rars import hst_stages
...
def _write_resource_pilot_artifacts(
    ...
    *,
    ...
    from covid_rars.hst_resource_pilot import select_base_resource_pilot
    ...
    from covid_rars.hst_resource_pilot import resource_pilot_freeze_payload
...
def test_resource_pilot_receipt_is_recomputed_and_binds_actual_producer_artifacts(
    ...
    from covid_rars.hst_training import verify_resource_pilot_receipt
...
def test_executable_allowlist_is_exact_and_content_verified(tmp_path: Path) -> None:
    from covid_rars.hst_training import verify_executable_allowlist
...
def test_confirmatory_seed_is_validated_against_protocol_context() -> None:
    from covid_rars.hst_training import build_training_execution_identity
...
def test_confirmatory_optimizer_covers_every_trainable_backbone_and_head_parameter_once() -> None:
    torch = pytest.importorskip("torch")
    from covid_rars.hst_training import _verify_full_finetuning_optimizer
...
def test_instantiated_model_audit_is_bound_to_actual_source_backbone(tmp_path: Path) -> None:
    torch = pytest.importorskip("torch")
    from covid_rars.hst_training import verify_initial_model_load_audit
...
def test_last_checkpoint_retains_generation_pinned_by_durable_progress(
    ...
    pytest.importorskip("torch")
    from covid_rars.hst_runtime import canonical_json_sha256
    from covid_rars.hst_training import _atomic_torch_save
...
def test_best_checkpoint_recovery_rejects_partial_next_epoch_state(
    ...
    from covid_rars.hst_training import _load_or_recover_best_checkpoint
...
def test_confirmatory_resume_refuses_invalid_artifacts_and_nonempty_fresh_start(
    ...
    from covid_rars.hst_training import _resolve_resume_checkpoint, train_hst_fold
...
def test_resume_accepts_only_last_checkpoint_role(tmp_path: Path, role: object) -> None:
    from covid_rars.hst_training import _atomic_torch_save, _resolve_resume_checkpoint
...
def test_optimizer_boundary_resume_matches_uninterrupted_training(
    ...
    import covid_rars.hst_training as training_module
