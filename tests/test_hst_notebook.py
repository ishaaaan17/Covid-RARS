...
def test_notebook_project_discovery_supports_override_and_ancestor_search() -> None:
    ...
    assert '(candidate / "src/covid_rars").is_dir()' in joined
...
def test_ubuntu_runbook_documents_gates_resume_and_non_guaranteed_targets() -> None:
    ...
    assert r"C:\Users\nhnis\Desktop\Covid-RARS" in text
    assert 'COVID_RARS_PROJECT_ROOT="${COVID_RARS_PROJECT_ROOT:-$HOME/Desktop/Covid-RARS}"' in text
    assert (
