...
def _hold_gpu_lease(
    ...
    from covid_rars.hst_runtime import acquire_gpu_token
...
def _hold_run_lease(
    ...
    from covid_rars.hst_runtime import acquire_run_lock
...
def test_parallel_preprocessing_rejects_result_from_another_request(
    ...
    import pandas as pd
    import covid_rars.hst_parallel as parallel
...
def test_atomic_json_write_replaces_complete_document_and_cleans_temp_files(
    ...
    from covid_rars.hst_runtime import atomic_write_json, read_json
...
def test_atomic_json_write_preserves_previous_document_if_replace_fails(
    ...
    import covid_rars.hst_runtime as runtime
...
def test_background_heartbeat_is_cpu_only_and_stops_cleanly(tmp_path: Path) -> None:
    from covid_rars.hst_runtime import HeartbeatEmitter, read_json
...
def test_gpu_uuid_cannot_escape_lease_root(tmp_path: Path) -> None:
    from covid_rars.hst_runtime import gpu_token_paths
...
def test_run_lock_records_config_and_rejects_duplicate_live_owner(
    ...
    from covid_rars.hst_runtime import acquire_run_lock, read_json
...
def test_source_tree_hash_is_order_independent_and_content_sensitive(
    ...
    from covid_rars.hst_runtime import source_tree_hash
...
def test_stage_fingerprint_is_canonical_and_tracks_upstream_changes() -> None:
    from covid_rars.hst_runtime import stage_fingerprint
...
def test_cpu_thread_environment_sets_all_native_thread_controls() -> None:
    from covid_rars.hst_runtime import CPU_THREAD_ENV_VARS, cpu_thread_environment
...
def test_cpu_thread_budget_applies_limiter_and_restores_environment() -> None:
    from covid_rars.hst_runtime import CPU_THREAD_ENV_VARS, cpu_thread_budget
...
def test_windows_gpu_root_is_stable_user_wide_and_never_pid_scoped(
    ...
    from covid_rars.hst_runtime import default_gpu_lease_root
    ...
    expected = (tmp_path / "local-app-data" / "covid_rars" / "hst_gpu").resolve()
...
def test_raw_input_hashes_are_required_and_invalidate_stage_fingerprints() -> None:
    from covid_rars.hst_runtime import STAGE_REQUIREMENTS, stage_fingerprint
...
def test_successful_receipt_rejects_invalid_output_paths_and_checksums(
    ...
    from covid_rars.hst_runtime import write_stage_receipt
...
def test_successful_launch_receipt_uses_same_output_validation(tmp_path: Path) -> None:
    from covid_rars.hst_runtime import write_launch_receipt
...
def test_exit_receipt_rejects_empty_run_id(tmp_path: Path) -> None:
    from covid_rars.hst_runtime import capture_process_identity, write_exit_receipt
