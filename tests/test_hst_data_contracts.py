...
def test_hst_contract_does_not_depend_on_legacy_coughvid_adapter() -> None:
    source = (
        Path(__file__).parents[1] / "src" / "covid_rars" / "hst_data_contracts.py"
...
def test_qualified_identifiers_prevent_cross_dataset_collision() -> None:
    from covid_rars.hst_data_contracts import qualify_identifiers
...
def test_prediction_contract_rejects_missing_fold_and_cross_fold_pooling() -> None:
    from covid_rars.hst_data_contracts import aggregate_to_participant, assert_prediction_key_contract
...
def test_audit_reports_prior_supervised_label_changes() -> None:
    from covid_rars.hst_data_contracts import audit_coughvid_labels, qualify_identifiers
...
def test_coughvid_label_audit_includes_expert_diagnosis_columns() -> None:
    from covid_rars.hst_data_contracts import audit_coughvid_labels
