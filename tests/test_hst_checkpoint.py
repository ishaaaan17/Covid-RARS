...
def test_download_verified_checkpoint_reuses_valid_regular_file_without_network(
    ...
    from covid_rars.hst_checkpoint import download_verified_checkpoint
...
def test_verify_hst_source_accepts_only_pinned_commit() -> None:
    from covid_rars.hst_checkpoint import verify_hst_source

    repo = Path(__file__).resolve().parents[1] / "HST"
...
def test_official_model_source_hash_is_pinned() -> None:
    from covid_rars.hst_checkpoint import EXPECTED_HST_MODEL_SOURCE_SHA256
...
def test_committed_source_hash_is_independent_of_checkout_line_endings(
    ...
    from covid_rars.hst_checkpoint import hst_model_source_sha256
...
def test_official_module_adapter_does_not_hide_unrelated_deprecations(
    ...
    from covid_rars.hst_checkpoint import _load_official_module
...
def test_official_checkpoint_files_match_frozen_hashes() -> None:
    ...
    if not all(path.is_file() for path in required):
        pytest.skip(
            "official HST checkpoints are optional; prepare them with the HST runbook"
        )
    verify_file(
        required[0],
