...
def default_gpu_lease_root(
    *,
    ...
    if platform_name == "nt":
        ...
        if local_app_data:
            return (
                Path(local_app_data) / "covid_rars" / "hst_gpu"
            ).resolve()
        ...
        return (
            Path.home() / "AppData" / "Local" / "covid_rars" / "hst_gpu"
        ).resolve()
    xdg_runtime = environment.get("XDG_RUNTIME_DIR")
    if xdg_runtime:
        candidate = Path(xdg_runtime)
        if candidate.is_dir() and os.access(candidate, os.W_OK):
            return candidate / "covid_rars" / "hst_gpu"
    uid = os.getuid()
    return Path("/var/tmp") / f"covid_rars-{uid}" / "hst_gpu"
