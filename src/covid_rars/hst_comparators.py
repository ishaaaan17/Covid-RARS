...
from covid_rars.hst_data_contracts import (
...
from covid_rars.hst_protocols import audit_hst_manifest
...
def _build_compare_is10_executable_recipe(
    ...
    *,
    ...
    module_source_root = root / "src" / "covid_rars"
...
def _git_clean_current_artifact(path: Path, root: Path, relative: Path, name: str) -> str:
    ...
    working_blob = _git_text(
        "hash-object", f"--path={relative_text}", "--", relative_text, cwd=root
    )
    if head_blob != working_blob:
        raise ValueError(f"{name} content differs from the current Git blob")
    return head_blob
