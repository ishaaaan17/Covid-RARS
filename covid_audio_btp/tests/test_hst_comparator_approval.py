...
def _project_repository(
    ...
    package_root = root / "covid_audio_btp"
    source_root = package_root / "src" / "covid_audio_btp"
...
def _stage_run(root: Path, manifest: pd.DataFrame) -> tuple[Path, Path]:
    run_id = "hst-" + "1" * 20
    run_root = root / "covid_audio_btp" / "data" / "outputs" / "hst" / run_id
...
def _candidate_inputs(
    ...
    import covid_audio_btp.hst_comparator_approval as approval
    import covid_audio_btp.hst_comparators as comparators
    ...
    features_path = root / "covid_audio_btp" / "data" / "processed" / "features.csv"
...
def _build_approval_candidate(
    ...
    output = Path(inputs["project_root"]) / "covid_audio_btp/reports/hst/candidates/approval.json"
...
def _build_accepted_candidate(
    ...
    output = root / "covid_audio_btp/reports/hst/candidates/accepted.json"
...
def _write_generation(
    ...
    features = pd.read_csv(
        root / "covid_audio_btp/data/processed/features.csv", low_memory=False
    )
...
def test_approval_candidate_binds_verified_stage_feature_recipe_and_pilot(
    ...
    from covid_audio_btp.hst_comparator_approval import build_comparator_approval_candidate
...
def test_generation_candidate_authenticates_receipt_files_and_approval_binding(
    ...
    manifest_path, current_path = _write_generation(
        ...
        accepted_path,
        accepted_hash,
        root / "covid_audio_btp" / "reports" / "hst" / "comparator_audit",
        monkeypatch,
    )
    output = root / "covid_audio_btp/reports/hst/candidates/generation.json"
    ...
    second = build_comparator_generation_acceptance_candidate(
        ...
        current_receipt_path=current_path,
        runtime_random_state=42,
        output_path=root / "covid_audio_btp/reports/hst/candidates/generation-second.json",
    )
...
def test_generation_acceptance_review_never_deserializes_unaccepted_pickle(
    ...
    candidate = build_comparator_generation_acceptance_candidate(
        ...
        current_receipt_path=current_path,
        runtime_random_state=42,
        output_path=root / "covid_audio_btp/reports/hst/candidates/no-pickle.json",
    )
