        ...
        if stage == "aligned_comparator":
            try:
                ...
                if not isinstance(identity, dict):
                    return False
                trusted_root = self.config.workspace_root.parent.resolve()
    ...
    for source_path in files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                ...
                elif node.module and node.module.startswith("covid_audio_btp."):
                    module_names.add(node.module.split(".", 2)[1])
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name.startswith("covid_audio_btp."):
    ...
    *,
    ...
    if mode in {"pilot", "full"}:
        ...
        try:
            ...
            errors.append(f"configured HST source escapes workspace: {hst_root}")
        checkpoint_root = (
            project_root
