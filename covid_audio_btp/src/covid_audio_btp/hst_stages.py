...
def _source_path(config: HSTPipelineConfig) -> Path:
    ...
    path = (config.workspace_root / str(supplied)).resolve()
    workspace_parent = config.workspace_root.parent.resolve()
    try:
        path.relative_to(workspace_parent)
    except ValueError as exc:
        raise ValueError(f"Configured HST source escapes the shared workspace: {supplied}") from exc
    return path
...
def _bind_frozen_audio_sources(
    ...
    project_root = pipeline.config.workspace_root.parent.resolve()
...
def _manifests(pipeline: HSTPipeline, _stage: str) -> Mapping[str, object]:
    ...
    seeds = (
        (52,)
        if pipeline.config.mode == "smoke"
        else tuple(int(value) for value in experiment.get("project_seeds", ()))
    )
...
def _comparator_trust_inputs(pipeline: HSTPipeline) -> dict[str, object]:
    ...
    expected_workspace = (trusted_root / "covid_audio_btp").resolve()
    if pipeline.config.workspace_root.resolve() != expected_workspace:
...
def _build_authenticated_fusion_binding(
    ...
    *,
    ...
    registry_receipt: dict[str, object] = {
        ...
        "registry_authority": "covid_audio_btp.HSTPipeline.receipt_chain",
