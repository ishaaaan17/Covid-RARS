...
\title{Temporal and Cross-Dataset Shifts Reveal Reliability Limits in Multimodal COVID-19 Respiratory-Audio Modeling}
...
Secondary analyses included repeated-seed training, a source-versus-target domain classifier, and overlap of two independently ranked top-800 feature sets from early and late periods. The latter used LightGBM gain rankings and Jaccard similarity $|A\cap B|/|A\cup B|$. The domain classifier was a standardized, class-balanced logistic regression evaluated with five-fold out-of-fold predictions on the 500 common features with highest source-training variance. Exploratory context models used standardized, class-balanced logistic regression to predict the source label from symptoms, demographics, date, country, duration, sample rate, and quality fields. Models were retrained after label permutation as an implementation-leakage control. Grouped permutation importance quantified the reduction in predictive performance after shuffling each variable group. Participant-aligned logistic models compared metadata-only, audio-only, and combined predictions on complete cases. Effect sizes, bootstrap intervals, and paired DeLong tests were reported. These small complete-case comparisons remained exploratory.

\section{Results}

\subsection{Cohorts and Source-Domain Performance}

The source and target differed substantially in class composition (Table~\ref{tab:cohorts}). Positive prevalence was 0.322 among known-label Coswara participants and 0.034 among external COUGHVID recordings. In the source participant split, the validation-selected equal-weight cough--speech fusion achieved AUROC 0.895, AUPRC 0.862, balanced accuracy 0.808, and F1 score 0.729 on 314 complete cases. Across five random states on the same test cohort, its AUROC was $0.8945\pm0.0068$; this measures fitting variation, not cohort-sampling uncertainty. An exploratory logistic stack reached 0.897, 0.863, 0.825, and 0.752, respectively, but did not replace the prespecified source reference. Neither multimodal estimate is directly comparable with the cough-only target.
...
\midrule
Validation-selected source multimodal fusion & AUROC / AUPRC & 314 & 0.895 / 0.862 & Equal-weight cough--speech mean. No multimodal cross-dataset test \\
Source fusion across seeds & Mean AUROC $\pm$ SD & 5 seeds & $0.8945\pm0.0068$ & Fixed test cohort; fitting variability, not sampling uncertainty \\
...
Protocol context changes the interpretation of published scores. HST reported COUGHVID internal AUROC $0.90\pm0.01$ after target-domain training and validation, whereas our 0.523--0.543 estimates measure frozen Coswara-to-COUGHVID portability; the values should not be ranked as if they shared a protocol \cite{aytekin2024hst}. In the closest published transfer direction, Islam \emph{et al.} reported approximately 0.53 under different cohort and balancing choices \cite{islam2026dndf}, consistent in magnitude with our four-model range.

Ganitidis \emph{et al.} explicitly modeled drift and adaptation in COVID-19 cough streams \cite{ganitidis2025drift}. We did not perform online adaptation. Instead, our retrospective temporal test documents the instability that would motivate monitoring, while its prevalence and feature-selection limitations prevent a prospective claim. More generally, the source-target separability and performance loss are consistent with health-AI work on acquisition-dependent shortcuts and dataset shift \cite{subbaswamy2020shift,ly2024shortcut}. Our findings support evaluating distribution-shift safeguards against clinically defined external targets rather than only generic non-cough sounds.
