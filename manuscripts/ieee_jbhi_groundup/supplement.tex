...
\title{Supplementary Material for ``Temporal and Cross-Dataset Shifts Reveal Reliability Limits in Multimodal COVID-19 Respiratory-Audio Modeling''}
...
Files were decoded to mono floating-point waveforms and resampled to 16~kHz. Recordings shorter than 0.50~s for cough or 1.00~s for breath and speech were marked short. A recording was marked mostly silent when more than 70\% of its samples lay outside the active interval returned by 35-dB trimming. It was marked clipped when more than 1\% of samples had absolute amplitude at least 0.999. The audit identified 23,539 usable recordings, 597 mostly silent recordings, 390 corrupt recordings, and 186 short recordings. Quality flags were determined before model fitting and describe file usability rather than disease status.
...
Quality duration & At least 0.50~s cough; 1.00~s breath or speech & Flags recordings too short for the corresponding task \\
Silence flag & More than 70\% outside the 35-dB active interval & Marks mostly silent recordings before modeling \\
Clipping flag & More than 1\% of samples at $|x|\geq0.999$ & Marks recordings with substantial digital clipping \\
...
\end{equation}
and combined $p_i=\sum_mw_mp_{im}$. Stacked fusion fitted class-balanced logistic regression to validation-set modality probabilities with a maximum of 2,000 iterations. The primary source rule was the prespecified uniform mean. For cough and speech its weights were therefore fixed at 0.5 and 0.5. Validation-AUPRC weighting and logistic stacking were exploratory alternatives whose parameters were estimated from validation predictions. Binary thresholds maximized validation balanced accuracy and were then frozen for source test evaluation.

Validation selected cough and speech as the source sound set. Their primary equal-weight mean reached AUROC 0.895 and AUPRC 0.862 on 314 complete-case source-test participants. The exploratory logistic stack reached AUROC 0.897 and AUPRC 0.863 but did not replace the primary rule. Both are source multimodal results. The cough-only COUGHVID experiment does not test either fusion.
...
Full context & Reported symptoms & 10 & 0.185 & Reported cough symptom \\
