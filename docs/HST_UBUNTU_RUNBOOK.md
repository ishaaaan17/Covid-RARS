...
`C:\Users\nhnis\Desktop\Covid-RARS`. Windows is for code review and Git synchronization. On Ubuntu, set `COVID_RARS_PROJECT_ROOT` to the local Covid-RARS checkout. Ubuntu is the scientific execution environment because it contains the datasets, approved artifacts, and NVIDIA GPU. Synchronize code through the reviewed Git branch; do not replace Ubuntu data or generated evidence with files from Windows.

After opening the notebook, confirm that `PROJECT_ROOT` resolves to the Ubuntu
repository root. All notebook and approval-script paths are derived from that
single root.
...
`.cache/hst/checkpoints`, the same directory used by production
